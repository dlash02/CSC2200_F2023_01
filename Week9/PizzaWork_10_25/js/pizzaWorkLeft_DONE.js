...
function updatePizzaDelivery() {
    ...
    for( let i=0; i <delKeys.length; i++ ){
        ...
        oStr += `<option value="${delKeys[i]}" ${checked}> ${delKeys[i]} </optipn> `;
    }
...
function updatePizza(){
    ...
    let pSize = document.getElementById("pizzaSize");
    let oStr = ""
    let pKeys = Object.keys(menu.pizza);
    let first = true;
